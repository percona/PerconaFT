MACRO(ADD_EXTRA_COMPILATION_FLAGS)
    ...
    if (CMAKE_COMPILER_IS_GNUCXX OR MINGW) #Not only UNIX but also WIN32 for MinGW
        ...
        set(WARNING_FALLTHROUGH "-Wimplicit-fallthrough=0")
        CHECK_C_COMPILER_FLAG(${WARNING_FALLTHROUGH} WARNING_FALLTHROUGH_ALLOWED)
        if (WARNING_FALLTHROUGH_ALLOWED)
            MESSAGE("Compiler flag ${WARNING_FALLTHROUGH} allowed")
            set(ACTIVATE_WARNING_FALLTHROUGH "ON" CACHE BOOL "activate -Wimplicit-fallthrough=0 flag")
        else ()
            MESSAGE("Compiler flag ${WARNING_FALLTHROUGH} not allowed")
        endif (WARNING_FALLTHROUGH_ALLOWED)
        ...
        if (ACTIVATE_WARNING_FALLTHROUGH)
            list(APPEND CMAKE_CXX_FLAGS ${WARNING_FALLTHROUGH})
            list(APPEND CMAKE_C_FLAGS ${WARNING_FALLTHROUGH})
        else ()
            string(REPLACE ${WARNING_FALLTHROUGH} "" CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS}")
            string(REPLACE ${WARNING_FALLTHROUGH} "" CMAKE_C_FLAGS "${CMAKE_C_FLAGS}")
        endif (ACTIVATE_WARNING_FALLTHROUGH)

...
// Exit:  This thread has permission to modify the output.
{
    ...
    r = toku_os_close(logger->fd);                               if (r!=0) return get_error_errno();
...
// Entry and Exit: This thread has permission to modify the output.
{
    ...
    if (logger->write_log_files) {
        logger->fd = toku_os_open(fname, O_CREAT+O_WRONLY+O_TRUNC+O_EXCL+O_BINARY, S_IRUSR+S_IWUSR);
        if (logger->fd==-1) {
        ...
        logger->fd = toku_os_open(DEV_NULL_FILE, O_WRONLY+O_BINARY, 0);
        if (logger->fd==-1) {
            return get_error_errno();
        }
    }
...
// Entry and exit: Holds no locks (this is called only during single-threaded activity, such as initial start).
{
    ...
    r = toku_os_close(logger->fd);                              assert(r == 0);
    logger->fd = -1;
...
static int peek_at_log (TOKULOGGER logger, char* filename, LSN *first_lsn) {
    int fd = toku_os_open(filename, O_RDONLY+O_BINARY, 0);
    if (fd<0) {
        int er = get_error_errno();

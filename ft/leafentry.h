...
int le_val_is_del(LEAFENTRY le, bool is_snapshot_read, bool is_read_committed_always, TOKUTXN txn); // Returns true if the value that is to be read is empty
...
// param: accept_outermost_committed - ignore f and simply return the value associated with the outermost committed XR
int le_iterate_val(LEAFENTRY le, LE_ITERATE_CALLBACK f, bool accept_outermost_committed, void** valpp, uint32_t *vallenp, TOKUTXN context);

...
volatile uint64_t toku_ftnode_verify_flags = 0;

void toku_set_ftnode_verify_flags(uint64_t verify_flags) {
    toku_ftnode_verify_flags = verify_flags;
}
    ...
    )
{
    ...
    if (node->height == 0) {
        if (toku_ftnode_verify_flags & 4)
            toku_verify_ftnode(ft, node);

...
static void test_padded_fit(void) {
    struct block_allocator::blockpair pairs[] = {
        ...
        block_allocator::blockpair(20 * alignment, 1 * alignment),
        // 128kb hole after bp[4]
        block_allocator::blockpair(117 * alignment, 1 * alignment),
        // 256kb hole after bp[4]
        block_allocator::blockpair(374 * alignment, 1 * alignment),
    };
    const uint64_t n_blocks = sizeof(pairs) / sizeof(pairs[0]);
    ...
    bp = block_allocator_strategy::padded_fit(pairs, n_blocks, 8000, alignment);
    assert(bp == &pairs[0]);
}

...
static void run_test (void) {
    ...
    CKERR(r);
    r = db->open(db, NULL, "hi", NULL, DB_BTREE, DB_AUTO_COMMIT|DB_CREATE, 0666);
    CKERR(r);
    r = db->close(db, 0);
    CKERR(r);
...
static void run_recover (void) {
...

    {
        ...
        r = db_create(&db, env, 0);
        CKERR(r);
        r = db->open(db, NULL, "hi", NULL, DB_BTREE, DB_AUTO_COMMIT, 0666);
        CKERR2(r, ENOENT);
        r = db->close(db, 0);
        CKERR(r);
    }

    {
        ...
        r = db_create(&db, env, 0);
        CKERR(r);
        r = db->open(db, NULL, "there", NULL, DB_BTREE, DB_AUTO_COMMIT, 0666);
        CKERR(r);
        r = db->close(db, 0);
        CKERR(r);
    }

...
#ident "Copyright (c) 2010-2015 Tokutek Inc.  All rights reserved."
#ident "$Id$"

// Inject an ENOSPC error during loader->close and see what happens.
...
static ssize_t pwrite_enospc(int UU(fd), const void *UU(p), size_t UU(n), toku_off_t UU(o)) {
    // toku_set_func_pwrite(NULL); need 2 ENOSPC errors to cause the crash
    errno = ENOSPC;
    return -1;
}
...
static void test_loader_create_close(int ndb) {
    ...
    toku_set_func_pwrite(pwrite_enospc);

    r = loader->close(loader); CKERR2(r, ENOSPC);

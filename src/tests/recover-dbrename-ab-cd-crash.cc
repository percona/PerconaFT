...
static void run_test (void) {
    ...
    assert(r == 0);
    {
        char dname[strlen(TOKU_TEST_FILENAME)+1+1+1];
        sprintf(dname, "%s/%s", TOKU_TEST_FILENAME, "a");
        r = toku_os_mkdir(dname, S_IRWXU+S_IRWXG+S_IRWXO);
        assert(r == 0);
    }
    {
        char dname[strlen(TOKU_TEST_FILENAME)+1+1+1];
        sprintf(dname, "%s/%s", TOKU_TEST_FILENAME, "c");
        r = toku_os_mkdir(dname, S_IRWXU+S_IRWXG+S_IRWXO);
        assert(r == 0);
    }
    ...
    CKERR(r);
    r = db->open(db, NULL, "a/b", NULL, DB_BTREE, DB_AUTO_COMMIT|DB_CREATE, 0666);
    CKERR(r);
    r = db->close(db, 0);
    CKERR(r);
...
static void run_recover (void) {
...

    {
        ...
        r = db_create(&db, env, 0);
        CKERR(r);
        r = db->open(db, NULL, "a/b", NULL, DB_BTREE, DB_AUTO_COMMIT, 0666);
        CKERR(r);
        r = db->close(db, 0);
        CKERR(r);
    }

    {
        ...
        r = db_create(&db, env, 0);
        CKERR(r);
        r = db->open(db, NULL, "c/d", NULL, DB_BTREE, DB_AUTO_COMMIT, 0666);
        CKERR2(r, ENOENT);
        r = db->close(db, 0);
        CKERR(r);
    }
